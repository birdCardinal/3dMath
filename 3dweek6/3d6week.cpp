...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        objTransform.rotation = glm::mat3(
            glm::cos(glm::radians((float)f)), -glm::sin(glm::radians((float)f)), 0,
            glm::sin(glm::radians((float)f)), glm::cos(glm::radians((float)f)), 0,
            0, 0, 1
        );
        ...
        for (int i = 0; i < 360; i++)
        {
            transformedCircle[i].pos = objTransform.translate * objTransform.rotation * objTransform.scale * circle[i].pos;
        }

        for (int i = 0; i < 5; i++)
        {
            transformedStar[i].pos = objTransform.translate * objTransform.rotation * objTransform.scale * star[i].pos;
        }
